...
const hbspath = path.join(__dirname, "views");
 ...
 app.use(express.static(rootpath + "/css"));
...
app.get("/listPlayers", (req, res) => {
  if (req.session.loggedin) {
    ...
    let queryTeams = `SELECT *
    FROM Team
    `
        let player = db.prepare(queryPlayers).all();
        let Team = db.prepare(queryTeams).all();
...
app.get("/profile", (req, res) =>  {
    if(req.session.loggedin) {
        res.render(hbspath + "/pages/profile.hbs")
    } else {
        res.sendFile(rootpath + "/login.html")
    }
})
...
app.post("/login", async (req, res) => {
  ...
  let userData = db
    .prepare("SELECT * FROM Player WHERE email = ?")
    .get(login.email);
